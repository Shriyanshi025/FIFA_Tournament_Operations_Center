...
export const CommunicationCenterPanel: React.FC = () => {
  ...
  // Autonomous AI Dispatch Copilot Chat Integration
  React.useEffect(() => {
    const lastMsg = messages[messages.length - 1];
    if (!lastMsg) return;
    
    // Ignore messages generated by the AI Copilot to avoid recursion loops
    if (lastMsg.content.includes("🤖")) return;
    
    // Ignore messages older than 6 seconds (initial history load)
    if (Date.now() - lastMsg.timestamp > 6000) return;

    const runAgentReply = async () => {
      telemetry.log("INFO", `AI Dispatch Copilot processing operational message from operator: "${lastMsg.senderName}"`);
      
      try {
        const response = await AIRequestManager.getInstance().executeRequest<any>({
          promptId: "ai-copilot-chat-reply",
          parameters: {
            senderName: lastMsg.senderName,
            senderRole: lastMsg.senderRole,
            chatMessage: lastMsg.content
          },
          priority: "HIGH"
        }, "google-gemini");

        const text = response.parsedData?.text || (response as any).rawResponse?.candidates?.[0]?.content?.parts?.[0]?.text;
        
        if (text) {
          await sendCollaborationMessage(
            "department_broadcast",
            `🤖 [AI Dispatch Copilot]: ${text.trim()}`,
            lastMsg.relatedIncidentId
          );
          telemetry.log("INFO", "AI Dispatch Copilot posted response to chat loop.");
        }
      } catch (err: any) {
        console.error("AI Copilot failed to reply:", err);
      }
    };

    const timer = setTimeout(runAgentReply, 1500);
              ...
              <div className="space-y-xs">
                {messages.length === 0 ? (
                  ...
                  messages.map((msg) => {
                    ...
                    const isAI = msg.content.includes("🤖");
                    ...
                    return (
                      <div 
                        ...
                        className={`p-2xs rounded-sm border text-left space-y-[2px] transition-colors ${
                          isAI
                            ? "bg-accent/5 border-accent/25 shadow-[0_0_8px_rgba(99,102,241,0.15)] animate-fade-in"
                            : isEmergency 
